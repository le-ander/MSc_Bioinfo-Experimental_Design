...
def mutInfo1SDE(dataMod,thetaMod,covMod,dataRef,thetaRef,covRef):
	...
	invcov = zeros((N4,S*T,S), dtype=float32)
	invdet = zeros((N4,T), dtype=float32)

	# Invert covariance matrices and calculate the determinant of the inverted matrices
	##Remove abs() for invdet
	for i in range(N4):
		for j in range(T):
			invcov[i,j*S:(j+1)*S,:] = linalg.inv(cov[N1+i,j*S:N1+(j+1)*S,:])
	...
	for i in range(numRuns_i):
		...
		for j in range(numRuns_j):
			...
			data_subset = ascontiguousarray(data[(i*int(grid_i)):(i*int(grid_i)+Ni),(j*int(grid_j)):(j*int(grid_j)+Nj),:,:])
