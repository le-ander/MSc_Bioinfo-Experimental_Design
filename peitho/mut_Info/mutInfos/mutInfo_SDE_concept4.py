...
def mutInfo1SDE(data,theta,cov):
	...
	__global__ void kernel_func2SDE(int n1, int b, double pre, float *invdet, double *x, double *mu, float *invcov, double *res1){
		...
		res1[idx2d(ti,tj,b)] = exp(res1[idx2d(ti,tj,b)]);
	}
	"""
	...
	for i in range(numRuns_i):
		...
		for j in range(numRuns_j):
			...
			data_subset1 = ascontiguousarray(data[(i*int(grid_i)):(i*int(grid_i)+Ni),(j*int(grid_j)):(j*int(grid_j)+Nj),:,:]) # d1 subunit for the next k runs
			...
			for k in range(numRuns_k):
				...
				theta_subset1 = theta[N1+(k*int(grid_k)):N1+(k*int(grid_k)+Nk),:,:]
				invcov_subset1 = invcov[N1+(k*int(grid_k)):N1+(k*int(grid_k)+Nk),:,:]
				invdet_subset1 = invdet[N1+(k*int(grid_k)):N1+(k*int(grid_k)+Nk),:]
				...
				gpu_kernel_func1SDE(int32(Ni), int32(Nj), int32(Nk), float64(pre), driver.In(invdet_subset1), driver.In(data_subset1),driver.In(theta_subset1), driver.In(invcov_subset1),driver.Out(res1), block=(int(bi),int(bj),int(bk)),grid=(int(gi),int(gj),int(gk)))
