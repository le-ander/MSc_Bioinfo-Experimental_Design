...
########################Calulation 2############################################
	...
	grid_prelim = launch.optimise_gridsize_ode(2, block_i, block_j, T, S)[0]
...
########################Final Computations######################################
	...
	count_inf1_prop = int(((count_inf1)/float(N1))*100)
	count_inf2_prop = int(((count_inf2)/float(N1))*100)
	print "Proportion of infs in 1st summation", count_inf1_prop, "% ("+str(count_inf1)+" infs)"
	print "Proportion of infs in 2nd summation", count_inf2_prop, "% ("+str(count_inf2)+" infs)"
	...
	return Info, count_all_inf, count_all_inf_prop
...
def run_mutInfo2(model_obj, input_SBML):
	...
	for experiment in range(model_obj.nmodels):
		...
		temp_list=mutInfo2(model_obj.trajectories[experiment],model_obj.cudaout[experiment],N1,N2,N3,model_obj.sigma,model_obj.scale[experiment])
		MutInfo_lists = [MutInfo2, MutInfo2_infs, MutInfo2_infs_prop]
	...
	return MutInfo2, MutInfo2_infs, MutInfo2_infs_prop