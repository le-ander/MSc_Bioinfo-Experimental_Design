...
def generateTemplate(source, filename="input_file", sumname="summary_file", dataname=None, inputpath="", outputpath=""):
	...
	***** kwargs *****
	...
	for i in range(0, len(source)):
		...
		document_check=reader.readSBML(inputpath + "/" + source[i])
	...
	out_file=open(outputpath+"/"+filename+".xml","w")
	sum_file=open(outputpath+"/"+sumname+".txt","w")
...
#    print vars
	...
	for j in range (0, len(fit_species)):
		...
		for i in range(0,len(source)):
			...
			document=reader.readSBML(inputpath + "/" + source[i])
