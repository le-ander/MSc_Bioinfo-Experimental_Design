...
struct myFex{
    __device__ void operator()(int *neq, double *t, double *y, double *ydot/*, void *otherData*/)
    {
        ...
        ydot[0]= -y[0] + (tex2D(param_tex,2,tid) / ( 1 + powf(y[5],tex2D(param_tex,0,tid)))) + tex2D(param_tex,1,tid);
        ydot[1]= -tex2D(param_tex,3,tid) * (y[1] - y[0]);
        ydot[2]= -y[2] + (tex2D(param_tex,2,tid) / (1 + powf(y[1],tex2D(param_tex,0,tid)))) + tex2D(param_tex,1,tid);
        ydot[3]= -tex2D(param_tex,3,tid) * (y[3] - y[2]);
        ydot[4]= -y[4] + (tex2D(param_tex,2,tid) / ( 1 + powf(y[3],tex2D(param_tex,0,tid)))) + tex2D(param_tex,1,tid);
