...
struct myFex{
    __device__ void operator()(int *neq, double *t, double *y, double *ydot/*, void *otherData*/)
    {
        ...
        ydot[4]=(1.0*(2.3*tex2D(param_tex,2,tid)+tex2D(param_tex,3,tid)/(1+__powf(y[3],tex2D(param_tex,1,tid))))-1.0*(y[4]))/tex2D(param_tex,0,tid);
        ydot[3]=(1.0*(tex2D(param_tex,4,tid)*y[2])-1.0*(tex2D(param_tex,4,tid)*y[3]))/tex2D(param_tex,0,tid);
        ydot[2]=(1.0*(0.7*tex2D(param_tex,2,tid)+7*tex2D(param_tex,3,tid)/(1+__powf(y[1],tex2D(param_tex,1,tid))))-1.0*(y[2]))/tex2D(param_tex,0,tid);
