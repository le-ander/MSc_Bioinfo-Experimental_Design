...
def sorting_files(input_file_SBML, input_file_data, analysis, fname, usesbml, which_species, parameter_change, init_condit):
	...
	if usesbml == True:
		...
		input_file_parser_new_2.generateTemplate(exp_xml_files, "input_xml", "summmary", input_file_data, inpath = inPath, outpath= xml_out)
		
		#print "-----Creating object from input XML file-----"
		#info_input_xml = parse_infoEnt_new.algorithm_info(xml_out+"/"+"input_xml.xml",0)
...
#p,s=obtain_thetas.THETAS(info_new, sampleGiven = False, sampleFromPost= "data_1.txt", weight= "w_1.txt", analysisType = 1, N1 = 2, N3 = 3, parameter_i = [0,3])
